# reco_service.py  ──────────────────────────────────────────────
import asyncio, io, os, heapq, pickle, redis
...
_ready_flag = asyncio.Event()                 # will be set once artefacts are ready

# globals filled by loader --------------------------------------------------
...
# ────────────────────────────────────────────────────────────────
# 1.  BACKGROUND LOADER (runs once)
# ────────────────────────────────────────────────────────────────
def _load_artefacts_sync() -> None:
    """Exactly the same heavy loading you had – but run in a worker thread."""
    ...
    # ── small CSV shipped inside the image -----------------------------------
    ...
    if os.path.exists("/assets/boost_table.csv"):
        adhoc = (pd.read_csv("/assets/boost_table.csv")
...
async def kick_off_loader() -> None:
    """Fire-and-forget the blocking load in a worker thread."""

    async def wrapper():
        try:
            await asyncio.to_thread(_load_artefacts_sync)
            _ready_flag.set()
        except Exception as exc:          # propagate loader errors
            print("❌  artefact load failed:", exc, flush=True)
            os._exit(1)                   # let Railway restart

    asyncio.create_task(wrapper())        # do *not* await → startup stays fast
...
@app.get("/")
async def root():
    return {"msg": "alive – see /docs", "ready": _ready_flag.is_set()}
...
def _cosine_row(i: int) -> np.ndarray:
    sims = (R.getrow(i) @ R.T).toarray().ravel()
    sims[i] = 0.0
...
def _user_side(user, target_sku, price_band):
    if user not in u2i or not valid_user(user):
        return {}
    ...
    for n in sims.argsort()[-200:][::-1]:
        ...
        for j in R[n, :].indices:
            s = j2s[j]
            if s == target_sku or not valid_sku(s):
                continue
            if price_band and not price_band(s):
                continue
            scores[s] += sims[n]
    return scores
...
def recommend(user: str, sku: str, *, top_n=20, pct_band=.30, w_user=.7):
    ...
    if len(combined) < top_n:  # back-fill
        pool = [s for s in META.index if valid_sku(s)
                and s not in combined and s != sku]
...
# ────────────────────────────────────────────────────────────────
# 4.  RECOMMEND ENDPOINT  (waits until ready)
# ────────────────────────────────────────────────────────────────
...
async def reco(req: RecoRequest):
    if not _ready_flag.is_set():
        raise HTTPException(503, "model loading – try again in a moment")

    try:
        recs = recommend(req.user,
                         req.sku,
                         top_n=req.top_n,
                         pct_band=req.pct,
                         w_user=req.w_user)
